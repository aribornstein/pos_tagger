...
class Tagger_Net(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        # tanh = tanh_func(self.linear1(self.d1(embeds)))
        # return self.linear2(self.d2(tanh))

def train_model(train_file, dev_file, outpath, epochs):
    """
    trains a tagging model
    """
    tags, trainloader, dev_x, dev_y = utils.generate_input_data(train_file, dev_file, pretrained=True)
...
if __name__ == "__main__":
    POS_MODEL, POS_TAGS = train_model(r'./pos/train', r'./pos/dev',
                                      './pos/train_embedded.model', utils.EPOCHS)    
    utils.write_model_results(POS_MODEL, r'./pos/test', POS_TAGS, 'test2.pos')

    # POS_VOCAB, POS_TAGS, _, _, _ = utils.generate_input_data(r'./pos/train', r'./pos/dev', pretrained=True)
    # POS_MODEL = torch.load('POS_tagger_embedded.model') 



    NER_MODEL, NER_TAGS = train_model(r'./ner/train', r'./ner/dev',
                                      'NER_tagger_embedded.model', utils.EPOCHS)
    utils.write_model_results(NER_MODEL, r'./ner/test', NER_TAGS, 'test2.ner')
